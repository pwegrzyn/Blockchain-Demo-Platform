...
kernel void sha256Kernel (global uint *data_info, global char *data, global uint *Hash, global uint *nounOffset) { // H for 8 hash word values
    ...
    for (i = 0; i < N; i++) {
        ...
        if (block > 0) {
            ...
            // Wt=Mt(i), 0<=t<=15
            for (t = 0; t < block/4; t++) {
                W[t] = ((uchar) getData(data, noun, data_info[2], used + t * 8)) << 24; // The first 8 bits (1 byte)
                W[t] |= ((uchar) getData(data, noun, data_info[2], used + t * 8 + 2)) << 16; // The second 8 bits (1 byte)
                W[t] |= ((uchar) getData(data, noun, data_info[2], used + t * 8 + 4)) << 8; // The third 8 bits (1 byte)
                W[t] |= (uchar) getData(data, noun, data_info[2], used + t * 8 + 6); // The last 8 bits (1 byte)
                //printf("W[%u]=%x\n", t, W[t]);
            }
            if (block%4 == 3) {
                W[t] = ((uchar) getData(data, noun, data_info[2], used + t * 8)) << 24; // The first 8 bits (1 byte)
                ...
                W[t] = ((uchar) getData(data, noun, data_info[2], used + t * 8)) << 24; // The 8 bits (1 byte)
